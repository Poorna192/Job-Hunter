...
def get_new_selector(html_content, element_description, old_selector):
    """
    Asks the Gemini AI to find a new CSS selector for a given element.
    """
    if not MODEL:
        return None

    # Truncate HTML to stay within token limits, focusing on the start of the body
    body_start = html_content.lower().find('<body')
    if body_start != -1:
        html_content = html_content[body_start:]
    
    html_snippet = html_content[:25000] # Use a significant chunk of the HTML
    ...
    try:
        ...
        # Basic validation to ensure it's a plausible selector
...
def update_selectors_file(site, element, new_selector):
    """Updates the scraper_selectors.json file with the new selector."""
